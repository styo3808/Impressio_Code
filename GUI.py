...
class GUI:
    ...
    def __init__(self, serialObject):
        ...
        self.magnet     = tk.StringVar(value="Drop")
    ...
    def initializeLabels(self):
        ...
        self.heightLabel.grid(row=0, column=0, sticky='nsew', columnspan=2)
    ...
    def initializeButtons(self, serialObject):
        ...
        ttk.Button(self.root, textvariable=self.magnet,
                    command=lambda: self.magnetToggle(serialObject)).grid(
    ...
    def magnetToggle(self, serialObject):
        """
        ...
        """
        current = self.magnet.get()
        if(current == "Drop"):
            self.magnet.set("Latch")
            serialObject.write(b'd');
        else:
            self.magnet.set("Drop")
    ...
    def exit(self, serialObject):
        ...
        if (current == "Drop"):
            messagebox.showinfo(title = 'Drop Warning', message = 'Drop the weight before exitting!!!')
        else:
    ...
    def updateHeight(self, height):
        ...
        if(currentUnit == "Metric"):
            ...
            self.height.set("Height: {} m".format(
                round(height / const.CONVERSION, 2)))
